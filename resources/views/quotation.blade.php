...
{{-- <form id="printForm" method="post" action="{{ route('show.invoice') }}" target="_blank">
    @csrf --}}
                    ...
                    <table class="table" style="width: 100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <th>{{ $pro->name }}</th>
                                <th>
                                    <form  action="{{ url('/quotation-cart-update/'.$pro->rowId) }}" method="post">
                                      @csrf
                                ...
                                <th>
                                  <form action="{{ url('/quotation-cart-updateprice/'.$pro->rowId) }}" method="post">
                                  @csrf
                                ...
                                <th><a href="{{ url('/quotation-cart-remove/'.$pro->rowId) }}"><i class="fas fa-trash"></i></a></th>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <form  method="post" action="{{ route('quotation-show.invoice') }}" target="_blank">
                   @csrf
                   <div class="row">
                    <div class="col-md-9">
                ...
                <div class="pricing-header bg-info">
                    ...
                    <a href="{{ route('quotation-clear-cart') }}" class="btn btn-danger">Clear Cart</a>
            ...
            <table id="datatable" class="table table-striped table-bordered" style="width:100%">
                ...
                <tbody>
                    @foreach($product as $row)
                        <tr>
                            <form action="{{ url('quotation-cart-add') }}" method="post">
                                @csrf
...
<!-- Add customer Modal -->
<form action="{{ url('/quotation-insert-customer-pos') }}" method="POST" enctype="multipart/form-data">
    @csrf
